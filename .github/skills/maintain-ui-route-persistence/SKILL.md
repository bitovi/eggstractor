---
name: maintain-ui-route-persistence
description: Provides an agent with the knowledge needed to safely modify, debug, or extend UI route persistence without breaking the read/write contract between the UI and main thread.
---
...
- [ ] Document what happens when `window.__INITIAL_ROUTE__` is present but malformed (e.g., invalid JSON from string substitution)
