...
export const spacingProcessors: StyleProcessor[] = [
  {
    ...
    process: async (
      ...
      const varValues: Partial<PaddingValues> = {
        top: variableMap.get('paddingTop')?.value,
        right: variableMap.get('paddingRight')?.value,
        bottom: variableMap.get('paddingBottom')?.value,
        left: variableMap.get('paddingLeft')?.value,
      };
